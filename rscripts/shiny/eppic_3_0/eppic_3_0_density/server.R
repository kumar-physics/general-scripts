...
all_ifaces=fetch(dbSendQuery(mydb,"select *,abs(cs1-cs2) dcs,abs(cr1-cr2) dcr from EppicTable where symmetry!='C39' and symmetry!='D48' and ftaxon1!='other sequences'"),-1)
...
shinyServer(function(input, output, session) {
  ...
  ifaces <- reactive({
    ...
    minh1 <- isolate(input$hmin1)
    maxh1 <- isolate(input$hmax1)
