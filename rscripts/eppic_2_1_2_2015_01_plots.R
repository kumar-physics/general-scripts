...
if(system("hostname",intern=T) == "delilah.psi.ch") { #spencer's system
  ...
  mydb=dbConnect(MySQL(),dbname="eppic_2015_01") #~/.my.cnf file configured with right username and passwd
}
...
op=fetch(dbSendQuery(mydb,"select operatorType,eppic,count(*) count 
                     from EppicTable where operatorType is not NULL 
                     group by operatorType,eppic;"),-1)
...
ep=fetch(dbSendQuery(mydb,"select pdbCode,interfaceId,resolution,rfreeValue,area,gm core,gmcall,crcall,cscall,eppic,
...
epvsaut=fetch(dbSendQuery(mydb,"select pdbCode,interfaceId,operatorType,area,gm,gmcall,cr,
                       crcall,cs,cscall,eppic,pisa,authors,pqs from EppicTable 
                       where resolution<2.5 and rfreeValue<0.3 and resolution>0 
                       and h1>30 and h2>30 and cscall!='nopred' and crcall!='nopred' and 
                       cscall=crcall and cscall=gmcall and authors is not NULL 
                       and get_chainlength(pdbCode,chain1)>50 and 
                       get_chainlength(pdbCode,chain2)>50 order by cs " ),-1)
ep2=rbind(epvsaut[1:10000,],epvsaut[(dim(epvsaut)[1]-10000):dim(epvsaut)[1],])

ep2$remark='No remark'
ep2$remark[ep2$authors=='xtal' & ep2$eppic=='xtal']<-'xtal xtal'
ep2$remark[ep2$authors=='bio' & ep2$eppic=='bio']<-'bio bio'
ep2$remark[ep2$authors=='bio' & ep2$eppic=='xtal']<-'xtal bio'
ep2$remark[ep2$authors=='xtal' & ep2$eppic=='bio']<-'bio xtal'
...
pdata2$issame[pdata2$authors==pdata2$eppic]="same call"
...
#ROC data
dc_cr<-subset(data,dataset=="dc" & crcall!="nopred" , select=c(cr,truth,crcall))
dc_cs<-subset(data,dataset=="dc" & cscall!="nopred",select=c(cs,truth,cscall))
dc_gm<-subset(data,dataset=="dc" & gmcall!="nopred",select=c(gm,truth,gmcall))
po_cr<-subset(data,dataset=="po" & crcall!="nopred",select=c(cr,truth,crcall))
po_cs<-subset(data,dataset=="po" & cscall!="nopred",select=c(cs,truth,cscall))
po_gm<-subset(data,dataset=="po" & gmcall!="nopred",select=c(gm,truth,gmcall))
many_cr<-subset(data,dataset=="many" & crcall!="nopred" & cr<500,select=c(cr,truth,crcall))
many_cs<-subset(data,dataset=="many" & cscall!="nopred" & cr<500,select=c(cs,truth,cscall))
many_gm<-subset(data,dataset=="many" & gmcall!="nopred" & cr<500,select=c(gm,truth,gmcall))

cs=roc(dc_cs$cs,dc_cs$truth,'dc')
cs=roc(po_cs$cs,po_cs$truth,'po',dat=cs)
cs=roc(many_cs$cs,many_cs$truth,'many',dat=cs)
cr=roc(dc_cr$cr,dc_cr$truth,'dc')
cr=roc(po_cr$cr,po_cr$truth,'po',dat=cr)
cr=roc(many_cr$cr,many_cr$truth,'many',dat=cr)
gm=roc_gm(dc_gm$gm,dc_gm$truth,'dc')
gm=roc_gm(po_gm$gm,po_gm$truth,'po',dat=gm)
gm=roc_gm(many_gm$gm,many_gm$truth,'many',dat=gm)
...
setwd('/home/baskaran_k/eppic_statistics/2015_01')
areavscore=ggplot(subset(eppic,gm>0,select =c(area,gm,eppic)))+
  geom_density2d(aes(x=area,y=gm,color=eppic),bins=5000,alpha=alpha_value)+
  ...
  theme(panel.background = element_blank(),
        ...
        legend.position='bottom');areavscore

areaplot=ggplot(subset(eppic,area<=5000),aes(x=area))+
  geom_histogram(aes(fill=eppic),binwidth=100,alpha=alpha_value,position="identity")+
  scale_color_manual(values=c(bio_color,xtal_color),name="Eppic final")+
  ...
  theme(panel.background = element_blank(),
        ...
        axis.text=element_text(color='black'),
        panel.border =element_rect(colour = "black",fill=NA),
        legend.position='bottom');areaplot

coreplot=ggplot(subset(eppic,area<=5000 & gm>0),aes(x=gm))+
  geom_histogram(aes(fill=eppic),binwidth=1,alpha=alpha_value,position="identity")+
  scale_color_manual(values=c(bio_color,xtal_color),name="Eppic final")+
  scale_fill_manual(values=c(bio_color,xtal_color),name="Eppic final")+
  ...
  theme(panel.background = element_blank(),
        ...
        axis.text=element_text(color='black'),
        panel.border =element_rect(colour = "black",fill=NA),
        legend.position='bottom');coreplot
...
opplot=ggplot(transform(op, operatorType = reorder(operatorType, -count)),aes(x=operatorType,y=count))+
  geom_bar(aes(fill=eppic),alpha=alpha_value,stat='identity')+
  scale_color_manual(values=c(bio_color,xtal_color),name="Eppic final")+
  scale_fill_manual(values=c(bio_color,xtal_color),name="Eppic final")+
  xlab('Operator type')+
  ylab('Count')+
...
auc_dc_cs<-auc(1-subset(roc_data, dataset=='dc' & method=='CoreSurface')$specificity,
               subset(roc_data,dataset=='dc' & method=='CoreSurface')$sensitivity)
auc_dc_cr<-auc(1-subset(roc_data, dataset=='dc' & method=='CoreRim')$specificity,
               subset(roc_data,dataset=='dc' & method=='CoreRim')$sensitivity)
auc_dc_gm<-auc(1-subset(roc_data, dataset=='dc' & method=='Geometry')$specificity,
               subset(roc_data,dataset=='dc' & method=='Geometry')$sensitivity)

auc_po_cs<-auc(1-subset(roc_data, dataset=='po' & method=='CoreSurface')$specificity,
               subset(roc_data,dataset=='po' & method=='CoreSurface')$sensitivity)
auc_po_cr<-auc(1-subset(roc_data, dataset=='po' & method=='CoreRim')$specificity,
               subset(roc_data,dataset=='po' & method=='CoreRim')$sensitivity)
auc_po_gm<-auc(1-subset(roc_data, dataset=='po' & method=='Geometry')$specificity,
               subset(roc_data,dataset=='po' & method=='Geometry')$sensitivity)

auc_many_cs<-auc(1-subset(roc_data, dataset=='many' & method=='CoreSurface')$specificity,
               subset(roc_data,dataset=='many' & method=='CoreSurface')$sensitivity)
auc_many_cr<-auc(1-subset(roc_data, dataset=='many' & method=='CoreRim')$specificity,
               subset(roc_data,dataset=='many' & method=='CoreRim')$sensitivity)
auc_many_gm<-auc(1-subset(roc_data, dataset=='many' & method=='Geometry')$specificity,
               subset(roc_data,dataset=='many' & method=='Geometry')$sensitivity)

rocplot=ggplot(roc_data)+
  facet_wrap(~method)+
  geom_line(aes(x=1-specificity,y=sensitivity,color=dataset),size=1.0)+
  scale_color_manual(values=c("#1b9e77","#d95f02","#7570b3"),name="Data set(AUC)",
                     breaks=c("dc", "po", "many"),
                     labels=c(sprintf("DC(%.2f,%.2f,%.2f)",auc_dc_cr,auc_dc_cs,auc_dc_gm),
                              sprintf("Ponstingl(%.2f,%.2f,%.2f)",auc_po_cr,auc_po_cs,auc_po_gm),
                              sprintf("Many(%.2f,%.2f,%.2f)",auc_po_cr,auc_po_cs,auc_po_gm)))+
  scale_linetype_manual(values=c(1,2,3),name="Method",
                        breaks=c("Geometry","CoreRim", "CoreSurface"),
                        labels=c("Geometry", "Core rim", "Core surface"))+
  theme(panel.background = element_blank(),
...
pdf('/home/baskaran_k/rocplot.pdf')
rocplot
dev.off()
...
nmrplot=ggplot(nmr)+
  ...
  theme(panel.background = element_blank(),
        ...
        panel.grid.minor = element_line(colour = "gray",linetype="dashed"),
        legend.title=element_blank(),
        legend.position='bottom');nmrplot
...
p2=ggplot()+
  geom_bar(dat=epvsaut,aes(x=cs,fill=cscall),,position='identity',bin='stat',binwidth=0.1,alpha=.5)+
  geom_bar(dat=ep2,aes(x=cs,fill=cscall),,position='identity',bin='stat',binwidth=0.1,alpha=.5)+
  scale_color_manual(values=c(bio_color,xtal_color),name="Eppic final")+
  scale_fill_manual(values=c(bio_color,xtal_color),name="Eppic final")+
...
pdf('/home/baskaran_k/auth_selection.pdf')
p2
dev.off()
...
benchmark_areaplot = ggplot(d) + 
  ...
  theme(panel.background = element_blank(),
        ...
        legend.title=element_blank(),
        legend.position='bottom'); benchmark_areaplot
benchmark_area_free = ggplot(subset(d,(area<=2000 & dataset=='many') | dataset=='dc' | dataset=='po')) + 
  facet_wrap(~dataset,scale='free')+
  geom_histogram(aes(x=area,fill=truth),binwidth=100,position='identity',alpha=alpha_value)+
  scale_fill_manual(values=c(bio_color,xtal_color))+
  scale_color_manual(values=c(bio_color,xtal_color))+
